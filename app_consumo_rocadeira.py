...
def seed_from_assets():
    ...
    try:
        if count_rows("modelos") == 0 and os.path.exists("assets/seed_catalogo.xlsx"):
            dfc = pd.read_excel("assets/seed_catalogo.xlsx", engine="openpyxl")
            with sqlite3.connect(DB_PATH) as conn:
                for _, r in dfc.iterrows():
                    conn.execute("""
                        INSERT OR IGNORE INTO modelos(marca, modelo, consumo_fabricante_l_h)
                        VALUES (?,?,?)
                    """, (str(r["marca"]), str(r["modelo"]), float(r["consumo_fabricante_l_h"]) if pd.notna(r["consumo_fabricante_l_h"]) else None))
        if count_rows("abastecimentos") == 0 and os.path.exists("assets/seed_abastecimentos.xlsx"):
            dfa = pd.read_excel("assets/seed_abastecimentos.xlsx", engine="openpyxl")
            with sqlite3.connect(DB_PATH) as conn:
                for _, r in dfa.iterrows():
                    conn.execute("""
                        ...
                        VALUES (?,?,?,?,?,?,?,?,?,?,?)
                    """, (str(r["data"])[:10], str(r["marca"]), str(r["modelo"]), str(r.get("equipamento","Roçadeira")),
                          float(r["litros"]), float(r["horas"]), float(r["area_valor"]), str(r["area_unidade"]),
...
with tab_reg:
    ...
    marcas = sorted(modelos_df["marca"].unique().tolist()) if not modelos_df.empty else []
    marca = st.selectbox("Marca da roçadeira", options=marcas if marcas else ["—"], key="marca_reg")
    modelos = modelos_df.loc[modelos_df["marca"]==marca, "modelo"].tolist() if marca != "—" and not modelos_df.empty else []
    ...
    if pd.notna(consumo_fab) and float(consumo_fab) > 0:
        st.success(f"Consumo médio do fabricante: **{float(consumo_fab):.2f} L/h**")
    else:
        st.info("Consumo do fabricante não cadastrado. Cadastre na aba **Catálogo**.")
...
with tab_imp:
    ...
    if up1 is not None:
        try:
            dfu = pd.read_excel(up1, engine="openpyxl")
            ok = 0
            with sqlite3.connect(DB_PATH) as conn:
    ...
    if up2 is not None:
        try:
            dfu = pd.read_excel(up2, engine="openpyxl")
            ok = 0
            with sqlite3.connect(DB_PATH) as conn:
                for _, r in dfu.iterrows():
                    conn.execute("""
